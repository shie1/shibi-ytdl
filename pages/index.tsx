...
const fetchWithProgress = async (url: string, params?: { timeout?: number, progressCallback?: (progress: number) => void, abortController?: AbortController }): Promise<Uint8Array | undefined> => {
...
const Home: NextPage = (props: any) => {
  ...
  useEffect(() => {
    ...
    fetch(video.thumbnailUrl).then(async (res: any) => {
      setBg(URL.createObjectURL(await res.blob()))
    })
  }, [video])
...
const DownloadOptionsModal = (
  {
    ...
    state: {
      ...
      dlProgress: { dlProgress: { video: number, audio: number }, setDlProgress: Dispatch<SetStateAction<{ video: number, audio: number }>> },
    }
  }
) => {
  // UI
  const [containerIsString, setContainerIsString] = useState<boolean>(false)
  const [videoStreamSizeBytes, setVideoStreamSizeBytes] = useState<number | undefined>(undefined)
  const [audioStreamSizeBytes, setAudioStreamSizeBytes] = useState<number | undefined>(undefined)
  ...
  // Retrive streams
  const videoStreams = !video ? [] : [{ // Retrive video streams
    label: 'No Video',
    value: "off"
  ...
  const audioStreams = !video ? [] : [{ // Retrive audio streams
    label: 'No Audio',
    value: "off"
  ...
  useEffect(() => {
    setVideoStreamSizeBytes(undefined)
  }, [videoStream])
  useEffect(() => {
    setAudioStreamSizeBytes(undefined)
  }, [audioStream])

  // Get stream sizes
  useEffect(() => { // Video stream size 
    const ac = new AbortController()
    if (!videoStreams.length) return
    let vs = videoStream || videoStreams[1].value
    if (!vs || vs === "off") { setVideoStreamSizeBytes(undefined); return }
    axios.head(vs, { signal: ac.signal }).then((res) => {
      setVideoStreamSizeBytes(parseInt(res.headers['content-length']))
    }).catch(() => { })
    return () => ac.abort()
  }, [videoStream, videoStreams])
  useEffect(() => { // Audio stream size
    const ac = new AbortController()
    if (!audioStreams.length) return
    let as = audioStream || audioStreams[1].value
    if (!as || as === "off") { setAudioStreamSizeBytes(undefined); return }
    axios.head(as, { signal: ac.signal }).then((res) => {
      setAudioStreamSizeBytes(parseInt(res.headers['content-length']))
    }).catch(() => { })
    return () => ac.abort()
  ...
  return <Modal
    ...
    onOk={async () => {
      ...
      const command = `-y${videoS}${audioS}${map} -f ${container.value} output-${conversionId}.${container.value}`
      console.log(command)
      notifications.open({
      ...
      setInProgress(false)
      try {
        const data = ffmpeg.FS('readFile', `output-${conversionId}.${container.value}`)
        ...
        const url = URL.createObjectURL(new Blob([data.buffer], { type: `${mediaType}/${container.value}` }))
        ...
        <div style={{ flex: 1, minWidth: 179 }}>
          ...
          <AnimatePresence>
            ...
            <div key="dlEst" style={{
              ...
              <Typography.Text>
                Estimated download time: {(videoStreamSizeBytes && audioStreamSizeBytes) ? Math.round(estimatedDownloadTimeSeconds) : '??'} seconds
              </Typography.Text>
            </div>
          </AnimatePresence>
        </div>
